...
#        logging.error("ants " + str(ants.my_ants()))
        ...
        for food_loc in ants.food():
            #if food_loc not in self.food.values():
            temp_ant = self.get_closest_ant(ants, food_loc)
            
            if temp_ant == None:
                temp_ant = self.get_closest_ant(ants, food_loc, self.mission["gather"])
                if (temp_ant != None and len(self.orders[temp_ant]) > 0 
                and self.ant_state[temp_ant] == self.mission["gather"] 
                and ants.distance(food_loc, self.ant_id[temp_ant]) 
                < ants.distance(self.orders[temp_ant][0], self.ant_id[temp_ant])):
                
                    self.ant_state[temp_ant] = self.mission["idle"]
            
            if temp_ant != None and self.ant_state[temp_ant] == self.mission["idle"]:
                logging.error("issuing orders to ant #" + str(temp_ant))
                self.orders[temp_ant] = self.a_star_search(ants, self.ant_id[temp_ant], food_loc)
                self.ant_state[temp_ant] = self.mission["gather"]
